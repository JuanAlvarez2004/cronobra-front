...
import { CalendarDays, ListTodo, LogOut, Plus, Users } from 'lucide-react'
import { Button } from '../ui/button'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '../ui/tabs'
import { CreateScheduleDialog } from '../features/schedules/CreateScheduleDialog'
...
import type { AppData, Schedule, Task, User } from '@routes/index'
    ...
    <div className="min-h-screen bg-slate-50">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-600">
                  Gestiona los cronogramas de obra
                </p>
              </div>
              <Button
          ...
          <TabsContent value="workers" className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {appData.workers.map((worker) => {
                const workerTasks = appData.tasks.filter(
                  (t) => t.assignedTo === worker.id,
                )
                const pendingTasks = workerTasks.filter(
                  (t) => t.status === 'PENDING',
                ).length
                const inProgressTasks = workerTasks.filter(
      ...
      {selectedTask && (
        <TaskDetailsDialog
          ...
          task={selectedTask}
          schedule={appData.schedules.find(
            (s) => s.id === selectedTask.scheduleId,
          )}
          worker={appData.workers.find((w) => w.id === selectedTask.assignedTo)}
          traceLog={appData.traceLog.filter(
