  ...
  if (schedules.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-lg border border-slate-200">
        ...
        <p className="text-sm text-slate-500">Crea tu primer cronograma para comenzar</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {schedules.map((schedule) => {
        ...
        const completedTasks = scheduleTasks.filter((t) => t.status === 'APPROVED').length;
        const progress =
          scheduleTasks.length > 0 ? (completedTasks / scheduleTasks.length) * 100 : 0;
