...
import { LogOut, HardHat, Clock, PlayCircle, CheckCircle } from 'lucide-react';
import { WorkerTaskList } from './WorkerTaskList';
import { UploadEvidenceDialog } from './UploadEvidenceDialog';
import type { User, AppData, Task } from '@routes/index';
...
export function WorkerDashboard({
  ...
  const completedTasks = myTasks.filter(
    (t) => t.status === 'COMPLETED' || t.status === 'APPROVED' || t.status === 'REJECTED'
  );
    ...
    setAppData({
      ...
      traceLog: [
        ...appData.traceLog,
        {
          ...
          user: currentUser.name,
          timestamp: now,
          details: 'Evidencia fotográfica subida - Tarea completada y esperando aprobación',
        },
      ],
    });
